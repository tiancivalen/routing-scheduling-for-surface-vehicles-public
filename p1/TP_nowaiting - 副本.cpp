...
//    }
    ...
    for(uint i=0; i<ndcount; i++){
        ...
        if(nd_id == curnodeid){
            ...
            avg_speed = getSpeed_nowaiting(currid,act);
...
//                    }
                    ...
                    if(!EntryWindow_Line_nowaiting(pnb->GetTWVector(dir_nb), pew, pnb->GetCapacity(),avg_speed, entw, awid_line)){
                        ...
                        //double cost1_delay = entw.tstart - in_bs->m_entw.tstart - cost1_time;////
                        ...
                        act = getAction_nowaiting(neighbor_zone_id,nd_id,nextinterid);
                        avg_speed = getSpeed_nowaiting(neighbor_zone_id,act);
                        ...
                        if(!ExitWindow_Line_nowaiting(entw, pnbaw, pnb->GetCapacity(), nnlength, pew, avg_speed, holdable)){
                            ...
                            if((nextinterid == 1000) || (g_model.matrix_nn_conn[nd_id][next_node_id] == 0)){//若nd_id的另一个相邻区域不存在，或当前路段不可用
                                ...
                                if(!Overlapped_nowaiting(pew, pnb_next->GetTWVector(), paircount, wtt)){
                                    continue;
                                }
                                else{
                                    ...
                                    for(itr=wtt.begin();itr!=wtt.end();itr++){
                                        ...
                                        if(Closed_nowaiting(bs,rsvec)){
                                            ...
                                            for(uint i=0; i<n; i++){
                                                ...
                                                assert(!b_s_tmp->Dominate_nowaiting(prs));
                                                if(prs->Dominate_nowaiting(b_s_tmp)){
                                                    flag = true;
                                                    break;
                                                }
                                            }
                                        ...
                                        if(Opened_nowaiting(bs, rsvec_open)){//判断是否open，并确定相应的base_step指针
                                            ...
                                            for(uint i = 0; i<n; i++){
                                                ...
                                                else if(b_s_tmp->Dominate_nowaiting(prs)){
...
//                }
    ...
    if(i == 0){//若当前规划航空器先于之前进入该路段的所有航空器进入该路段
        tl = in_nnlength/in_speed + in_entw.tstart;
        rOccVariable* ptmp;
        double time_tmp;
...
void UpdateTimeWindowVector_nowaiting(Path& in_path, CRegionCollect &rct)
{
    ...
    for(uint i = 0; i<n-1; i++){
        ...
        avg_speed = getSpeed_nowaiting(rngid,act);
...
void HoldDetection_nowaiting()
{
    ...
    for(uint i=0; i<g_vehs.size(); i++){
        ...
        for(uint j=1; j<pveh->m_path.size();j++){
            ...
            speed = getSpeed_nowaiting(rgnid,act);
//            if(g_model.matrix_nn_conn[ndin][ndout]==0){
//                int xx = 0;
//            }
...
double getSpeed_nowaiting(uint rgnid, uint action)
{
...
void printPathDetail_nowaiting(Path& rpath, QString fdir)
{
    ...
    if(!file.open(QFile::WriteOnly | QFile::Text)){
        ...
        //time | node | region | delay |[entw.tstart,entw.tend]
        QString strline = QString("%1 %2 %3 %4 %5 %6").arg(rs->m_entrytime,0,'f',3)
                .arg(GetNodeIndex(rs->m_bs)).arg(GetRegionIndex(rs->m_bs)).arg(rs->m_entrytime - g_pveh->m_start_time)
                .arg(rs->m_entw.tstart,0,'f',3).arg(rs->m_entw.tend,0,'f',3);//第一行输出的等待时间是delay_start
        ts << strline << '\n';
        ...
        for(uint i=1;i<rpath.size();i++){
            ...
            //time | node | region | [entw.tstart,entw.tend]
            strline = QString("%1 %2 %3 %4 %5 %6").arg(rs_next->m_entrytime,0,'f',3)
                    .arg(GetNodeIndex(rs_next->m_bs)).arg(GetRegionIndex(rs_next->m_bs)).arg(delay,0,'f',3)
                    .arg(rs_next->m_entw.tstart,0,'f',3).arg(rs_next->m_entw.tend,0,'f',3);
            ts << strline << '\n';
